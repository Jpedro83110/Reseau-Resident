...
export default async function handler(req, res) {
  res.setHeader('Access-Control-Allow-Origin', 'https://reseau-resident.fr');
  ...
  const resendFrom =
    process.env.RESEND_FROM_EMAIL || 'Carte Résident <onboarding@resend.dev>';
  ...
  try {
    ...
    if (!carte_id) {
      return res.status(400).json({ error: 'Missing carte_id' });
    }
    ...
    if (!carte.email) {
      return res.status(400).json({ error: 'Carte has no email' });
    }
    ...
    const villeName = !villeError && ville?.nom ? ville.nom : '';

    const isDigital =
      carte.type_carte === 'digitale' || carte.type_carte === 'les_deux';

    const scanUrl = carte.qr_token
      ? `https://reseau-resident.fr/scan?token=${encodeURIComponent(carte.qr_token)}`
      : null;
  ...
  <div style="max-width:600px;margin:0 auto;padding:40px 20px;">
    ...
    if (!response.ok) {
      ...
      return res.status(500).json({
        error: 'Email failed',
        detail: err?.message || 'Unknown Resend error',
      });
    }
    ...
    if (updateError) {
      ...
      return res.status(500).json({
        error: 'Email sent but database update failed',
      });
    }
