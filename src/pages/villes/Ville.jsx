        ...
        <div className="bg-white rounded-3xl p-8 md:p-12 shadow-xl border border-gray-100 max-w-xl mx-auto">
          {sent ? (
            ...
            <>
              ...
              <p className="text-gray-600 mb-8">Laissez votre email pour être prévenu dès le lancement de la Réseaux-Résident à {ville.nom}.</p>
...
function VilleActive({ ville }) {
  ...
  return (
    <div className="min-h-screen bg-creme"
      style={{
        '--ville-primaire': ville.couleur_primaire || '#1a3a5c',
        '--ville-secondaire': ville.couleur_secondaire || '#c8963e',
      }}>
      {/* Hero avec couleurs dynamiques */}
      <section className="relative pt-32 pb-24 text-white overflow-hidden"
        style={{ background: `linear-gradient(135deg, ${ville.couleur_primaire || '#1a3a5c'}, ${ville.couleur_primaire ? ville.couleur_primaire + 'cc' : '#0d2440'})` }}>
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-8 mb-12">
            <div className="flex items-center gap-6">
              ...
              <div>
                ...
                <p className="text-xl text-white/70 font-serif italic">{ville.departement}</p>
              </div>
            </div>
            <Link
              to={`/inscription?ville=${ville.slug}`}
              className="inline-flex items-center justify-center px-8 py-4 text-lg font-bold rounded-xl text-white transition-colors shadow-lg whitespace-nowrap"
              style={{ background: ville.couleur_secondaire || '#c8963e' }}
            >
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {statsMensuelles && statsMensuelles.length > 0 && (
            <div className="bg-white rounded-3xl p-8 shadow-sm border border-gray-100 mb-16">
              ...
              <p className="text-center text-sm text-gray-500 mt-4 italic">Évolution des visites générées par la Réseaux-Résident</p>
            </div>
          )}
          ...
          {evenements.length > 0 && (
            <div className="bg-white rounded-3xl p-8 shadow-sm border border-gray-100 mb-16">
              <div className="flex items-center justify-between mb-6">
                <h3 className="font-serif text-2xl font-bold text-texte">Événements à venir</h3>
                <Link to={`/villes/${ville.slug}/evenements`} className="text-sm text-bleu hover:text-bleu-clair font-medium transition-colors">Voir tout →</Link>
              </div>
          ...
          <div className="bg-gradient-to-r from-gray-50 to-white rounded-3xl p-8 md:p-12 border border-gray-200 flex flex-col md:flex-row items-center justify-between gap-8">
            <div>
              ...
              <p className="text-lg text-gray-600 max-w-xl">Rejoignez gratuitement le réseau Réseaux-Résident à {ville.nom} et attirez une nouvelle clientèle locale.</p>
            </div>
            <Link
...
export default function Ville() {
  ...
  usePageMeta(ville?.nom, ville?.nom ? `Découvrez les commerces et événements de ${ville.nom} sur Réseaux-Résident.` : undefined);

  useEffect(() => {
    if (ville?.slug) trackEvent('ville_page_viewed', { slug: ville.slug, nom: ville.nom });
  }, [ville?.slug]);
