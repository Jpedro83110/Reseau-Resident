...
function StepInfos({ formData, setFormData, onNext, onPrev, villesActives }) {
  const Field = ({ label, name, type = 'text', placeholder, required = true }) => (
    <div>
      <label htmlFor={name} className="block text-sm font-bold text-gray-700 mb-2">{label}{required && ' *'}</label>
      <input id={name} type={type} name={name} value={formData[name]}
    ...
    <div className="p-8 md:p-12">
      ...
      <div className="space-y-6 mb-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <Field label="Téléphone" name="telephone" type="tel" placeholder="06 00 00 00 00" required={false} />
        </div>
        ...
        <div>
          <AutocompleteVille
            ...
            onChange={(val) => setFormData((p) => ({ ...p, villeNom: val }))}
            onSelect={(v) => {
              const match = villesActives.find((va) => va.nom.toLowerCase() === v.nom.toLowerCase());
              if (match) {
                setFormData((p) => ({ ...p, ville: match.slug, villeNom: match.nom }));
              } else {
                setFormData((p) => ({ ...p, ville: '', villeNom: v.nom }));
              }
            }}
          ...
          {formData.villeNom && !formData.ville && (
            <p className="text-sm text-orange-600 mt-2">Cette ville n'est pas encore active. Vous pourrez vous inscrire dès qu'un commerce partenaire y sera validé.</p>
          )}
        </div>
        ...
        <div className="pt-4 border-t border-gray-100">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            {[
              { value: 'physique', label: 'Carte physique', desc: 'Carte plastifiée dans votre portefeuille', icon: <CardIcon size={20} /> },
              { value: 'digitale', label: 'Carte digitale', desc: 'QR code sur votre téléphone', icon: <Smartphone size={20} /> },
              { value: 'les_deux', label: 'Les deux', desc: 'Physique + QR code digital', icon: <Check size={20} /> },
            ].map((opt) => (
              <label key={opt.value}
        ...
        {/* Livraison (seulement si carte physique) */}
        {formData.typeCarte !== 'digitale' && (
          <div className="pt-4 border-t border-gray-100">
            <h3 className="font-bold text-lg mb-4">Livraison de votre carte physique</h3>
            <div className="space-y-3">
              {[
                { value: false, label: 'Envoi par courrier postal (gratuit)', desc: 'Recevez votre carte sous 5 jours ouvrés.' },
                { value: true, label: 'Retrait chez un commerçant partenaire', desc: 'Récupérez votre carte immédiatement.' },
              ].map((opt) => (
...
function StripeForm({ formData, carteId, onSuccess, onPrev }) {
  ...
  async function handleSubmit(e) {
    ...
    if (paymentIntent?.status === 'succeeded') {
      try { await confirmerPaiement(carteId, paymentIntent.id); onSuccess(); }
      catch { setError('Erreur lors de la confirmation. Contactez-nous.'); }
    }
    setLoading(false);
  }
      ...
      <div className="flex justify-between pt-4">
        ...
        <button type="submit" disabled={!stripe || loading}
          ...
          {loading ? (<><div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin" /> Traitement...</>)
            : (<><CreditCard size={20} aria-hidden="true" /> Payer {STRIPE_PRICES[formData.formule]?.montant / 100}€</>)}
        </button>
      </div>
    </form>
  );
...
function StepPaiement({ formData, setFormData, onSuccess, onPrev }) {
  ...
  // Use ref to capture formData at mount time (avoids stale closure)
      ...
      <div className="flex flex-col md:flex-row gap-10">
        <div className="md:w-64 shrink-0">
          ...
          <div className="mt-4 bg-blue-50 rounded-2xl p-5 border border-blue-100 text-sm text-bleu">
            <div className="flex items-center gap-2 font-bold mb-2"><Phone size={16} aria-hidden="true" /> Par chèque</div>
...
function StepConfirmation({ formData }) {
  const qrUrl = formData.qrToken ? `${window.location.origin}/scan?token=${formData.qrToken}` : null;
  ...
  return (
    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="p-8 md:p-16 text-center">
      ...
      <p className="text-xl text-gray-600 mb-8 max-w-lg mx-auto">
        Votre Carte Résident est en cours de préparation. Un email de confirmation vous a été envoyé à <strong>{formData.email}</strong>.
      </p>
      {formData.numeroCarte && (
        <div className="bg-gray-50 rounded-2xl p-8 max-w-sm mx-auto mb-6 border border-gray-100">
          <div className="text-xs text-gray-500 uppercase tracking-widest font-bold mb-2">Votre Carte Résident</div>
          <div className="font-mono text-2xl font-bold text-bleu tracking-[0.15em] mb-3">{formData.numeroCarte}</div>
          {formData.villeNom && (
            <div className="text-sm text-gray-500">Ville : <strong className="text-texte">{formData.villeNom}</strong></div>
          )}
        </div>
      )}
      {qrUrl && (formData.typeCarte === 'digitale' || formData.typeCarte === 'les_deux') && (
        <div className="bg-blue-50 rounded-2xl p-6 max-w-sm mx-auto mb-6 border border-blue-100">
          <div className="text-xs text-bleu uppercase tracking-widest font-bold mb-3">Votre carte digitale</div>
          <p className="text-sm text-gray-600 mb-3">Montrez ce QR code aux commerçants partenaires ou accédez-y depuis votre lien personnel :</p>
          <div className="bg-white rounded-xl p-4 border border-gray-200">
            <p className="font-mono text-xs text-bleu break-all">{qrUrl}</p>
          </div>
          <p className="text-xs text-gray-400 mt-3">Sauvegardez ce lien dans vos favoris pour y accéder facilement.</p>
        </div>
      )}
      <Link to={`/villes/${formData.ville}`}
  ...
  useEffect(() => {
    ...
    getVilles().then((villes) => {
      if (!cancelled) {
        ...
        // Si une ville était passée en URL param, résoudre son nom
